...
import AugmentedStream from "./components/AugmentedStream";
    ...
    render = () => {
        ...
        if (!state.browserCheckPassed) {
            return (
                <Screensaver
                    ...
                    message={
                        "You  must be on a mobile or tablet device to continue to Novastack.app"
                    }
                />
            );
        ...
        if (state.isWaitingForModel) {
            return (
                <ModelMenu
                    models={models}
                    onModelChoose={this.handleModelChoose}
                />
            );
        }
        if (state.isWaitingForDevice) {
            return (
                <DeviceMenu
                    onDeviceChoose={this.handleDeviceChoose}
                    stream={this.stream}
                />
            );
        }
        if (state.isWaitingCamera) {
            ...
            return (
                <Screensaver icon="../icons/error.svg" message={state.error} />
            );
        } else {
            return (
                <Screensaver icon="../icons/trolley.svg" message="Loading." />
            );
        }
    };
}
