...
function bootstrap(module) {
    requestMediaDevice().then(success).catch(cameraError);
...
function init(module) {
    ...
    const onInitDef = module.cwrap("onInitDef", null, [
        "number",
        "number",
        "number",
    ]);
    const addMarker = module.cwrap("addMarker", null, [
        "number",
        "number",
        "number",
    ]);
    const onProcess = module.cwrap("onProcess", "number", [
        "number",
        "number",
        "number",
        "number",
...
const updateSaver = (status) => {
    ...
    if (status == "camera") {
        ...
        message =
            "Cant't get access to device camera. Disable ad blocker or try to update your browser.";
    } else if (status == "cameraError") {
        iconSrc = "./icons/error.svg";
        message =
            "Can't get access to your web-camera, please check its connection and reload this page.";
        ...
        function success(stream) {
            ...
            navigator.mediaDevices
                ...
                .then((devices) => {
                    ...
                    document
                        .querySelector(".saver-content")
                        .appendChild(deviceList);
                })
                .then(() => {
